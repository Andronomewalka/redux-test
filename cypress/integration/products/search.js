describe("Search products", () => {
  ...
  it("Search changes products list", () => {
    ...
    cy.intercept(
      "GET",
      /\/products\?.*name_like=google|description_like=google.*$/,
      fakeResponses[0]
    ).as("getSearchProducts");

    cy.intercept(
      "GET",
      /\/products\?.*name_like=android|description_like=android.*$/,
      fakeResponses[1]
    ).as("getSearchNewProducts");
    ...
    cy.get("@search")
      ...
      .then(($productsList) => {
        ...
        cy.get("@search")
          .clear()
          .type(searchNewValue)
          .wait("@getSearchNewProducts")
          .get("@products")
          .children()
          .should("have.length", 1)
          .and("not.equal", $productsList)
          .then(($newProductsList) => {
  ...
  it("Next prev buttons works", () => {
    ...
    cy.get("@pageInput")
      .invoke("val")
      .then(($beforePrevInputValue) => {
        cy.get("[data-cy=prev-page-btn]")
          .click()
          .get("@pageInput")
          .should("have.value", +$beforePrevInputValue - 1);
      });

    cy.get("@pageInput")
      .invoke("val")
      .then(($beforeNextInputValue) => {
        cy.get("[data-cy=next-page-btn]")
          .click()
          .get("@pageInput")
          .should("have.value", +$beforeNextInputValue + 1);
      });
  });
});
